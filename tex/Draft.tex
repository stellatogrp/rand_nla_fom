...
\begin{theorem}\label{thm:lyapunov-inequality}
    ...
    Then, 
    \begin{align}
        \|z^{(k+1)} - z^\star\|_M^2 \leq \|z^{(k)} - z^\star\|_M^2 - \| q^{(k+1)}\|_M + \|\varepsilon^{(k+1)}\|_M^2 \label{eq:one-step-inequality-no-relaxation}.
    \end{align}
    Note that $\varepsilon^{(k)}$ represents the error in the linear system at iteration $k$.
    \begin{proof}
        Define
        \begin{align}
            s^{(k+1)} &= \begin{bmatrix}
...
\begin{remark}
    Under the assumption that $\sup_k \frac{\|s^{(k,\star)}\|_M}{\|s^{(k+1,\star)}\|_M} \leq D < \infty$ (where $D$ is some constant), then we also get a per-iteration bound
    \[\mathbb{E}[N_k] \leq \frac{(\mathcal{C}C^2}{\sigma^2(1-a)}.\]
